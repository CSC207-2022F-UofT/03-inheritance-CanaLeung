...
public abstract class Bag {
    ...
    public boolean addItem(String item){
        if (this.numberOfContents < this.capacity){
            ...
            List<String> tempContents = Arrays.asList(this.contents);
            int l = this.contents.length;
            tempContents.add(item);
            this.contents = tempContents.toArray(new String[l]);
    ...
    public String popItem(){
        if (this.numberOfContents == 0){
            ...
            List<String> tempContents = Arrays.asList(this.contents);
            int l = tempContents.size();
            String removed = this.contents[l];
            tempContents.remove(l);
            this.contents = tempContents.toArray(new String[l-1]);
            this.numberOfContents--;
            return removed;
        }
    }
